...
public class BundleContextExtensionTest {
	...
	private static <R> Stream<DynamicNode> runMultilevelTestClass(String type,
		...
		MultiLevelCleanupTest.getGlobalResourceChecker(testClass)
			.assertNotSetup(beforeSoftly, BEFORE_CLASS, EnumSet.allOf(CallbackPoint.class));
		...
		MultiLevelCleanupTest.getGlobalResourceChecker(testClass)
			.assertNotSetup(beforeSoftly, AFTER_CLASS, EnumSet.allOf(CallbackPoint.class));
	...
	static InputStream getBundleForCallbackPoint(CallbackPoint callbackPoint) {
		return getBundle(callbackPoint
			.toString()
			.replace(".", "/") + ".jar");
	}
	...
	static class BundleChecker extends BundleContextResourceChecker<Bundle> {
		...
		@Override
		public void doAssertSetup(CallbackPoint currentPoint, CallbackPoint pointThatShouldBeSetup, Bundle installedBundle) {
	...
	static class ServiceResourceChecker extends BundleContextResourceChecker<ServiceRegistration<String>> {
		...
		public ServiceRegistration<String> doSetupResource(CallbackPoint currentPoint) {
			return bc.registerService(String.class, "Service for callback point "
				+ currentPoint,
				dictionaryOf("multileveltest.scope", currentPoint.toString()));
		}

		@Override
		public void doAssertSetup(CallbackPoint currentPoint,
			CallbackPoint pointThatShouldBeSetup,
			ServiceRegistration<String> registration) {
			assertThat(registration).isNotNull();
			try {
				System.err.println("registered services: " + bundle.getRegisteredServices());
				ServiceReference<String> reference = registration.getReference();
				assertThat(bundle.getRegisteredServices()).as(currentPoint + ": checking resource from "
					+ pointThatShouldBeSetup
					+ "\n")
		...
		@Override
		public void doAssertNotSetup(CallbackPoint currentPoint,
			CallbackPoint pointThatShouldNotBeSetup,
			ServiceRegistration<String> registration) {
			assertThat(registration).isNotNull();
			ServiceReference<?>[] registeredServices = bundle.getRegisteredServices();
			try {
				registration.getReference();
				fail(String.format("[%s: checking resource from %s]:\nservice was not unregistered: %s ",
					currentPoint,
	...
	public Stream<DynamicNode> cleansUpServiceListenersMultiLevel() throws Exception {
		...
		try {
			...
			BundleContextMultiLevelCleanupTest.setFactory((BundleContext bc,
				Map<CallbackPoint, ServiceListener> map) -> new ServiceListenerResourceChecker(bc, map, installedBundle));
	...
	static void dumpStack(int lineCount) {
		StackTraceElement[] stack = Thread.currentThread()
			.getStackTrace();
		Stream.of(stack)
			.filter(x -> x.getClassName()
				.equals(MultiLevelCleanupTest.class
					.getName()))
			.map(s -> "    at " + s)
			.forEach(System.err::println);
		// int max = lineCount > stack.length ? lineCount : stack.length;
	...
	static class GottenServicesResourceChecker extends BundleContextResourceChecker<ServiceReference<Foo>> {
		...
		public ServiceReference<Foo> doSetupResource(CallbackPoint currentPoint) throws InvalidSyntaxException {
			ServiceReference<Foo> reference = bc.getServiceReferences(Foo.class, "(test.scope="
				+ currentPoint
				+ ")")
				.iterator()
				.next();
			dumpStack(12);
			System.err.println("service reference: " + currentPoint + ", " + reference);
		...
		public void doAssertSetup(CallbackPoint currentPoint,
			...
			assertThat(reference).as(String.format("%s checking resource from %s setup: reference",
				currentPoint,
				pointThatShouldBeSetup))
				.isNotNull();
			assertThat(bundle.getServicesInUse()).as(String.format("%s checking resource from %s setup: inUse",
				currentPoint,
				pointThatShouldBeSetup))
				.isNotNull()
				.contains(reference);
		...
		public void doAssertNotSetup(CallbackPoint currentPoint,
			CallbackPoint pointThatShouldNotBeSetup,
			ServiceReference<Foo> reference) {
			assertThat(reference).as(String.format("%s checking resource from %s not setup: reference",
				currentPoint,
				pointThatShouldNotBeSetup))
				.isNotNull();
	...
	static class GottenServiceObjectsResourceChecker extends BundleContextResourceChecker<ServiceObjects<Foo>> {
		GottenServiceObjectsResourceChecker(BundleContext bc, Map<CallbackPoint, ServiceObjects<Foo>> resources) {
			super(bc, resources);
		}

		@Override
		public ServiceObjects<Foo> doSetupResource(CallbackPoint currentPoint) throws InvalidSyntaxException {
		...
		public void doAssertSetup(CallbackPoint currentPoint, CallbackPoint pointThatShouldBeSetup,
			...
			assertThat(services).as(String.format("%s checking resource from %s setup: services",
				currentPoint,
				pointThatShouldBeSetup))
				.isNotNull();
			assertThat(bundle.getServicesInUse())
				.as(String.format("%s checking resource from %s setup: inUse", currentPoint,
					pointThatShouldBeSetup))
				.isNotNull()
		...
		public void doAssertNotSetup(CallbackPoint currentPoint,
			CallbackPoint pointThatShouldNotBeSetup,
			ServiceObjects<Foo> services) {
			assertThat(services).as(String.format("%s checking resource from %s not setup: services",
				currentPoint,
				pointThatShouldNotBeSetup))
				.isNotNull();
			ServiceReference<?>[] inUse = bundle.getServicesInUse();
			if (inUse != null) {
				assertThat(inUse).as(String.format("%s checking resource from %s not setup: inUse", currentPoint,
					pointThatShouldNotBeSetup))
