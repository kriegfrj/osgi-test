...
abstract class MultiLevelCleanupTest {
	...
	static void staticVerifyCurrentCallbackPoint(Class<? extends MultiLevelCleanupTest> testClass,
		...
		// ... check that the remaining are *not* set up...
		staticAssertNotSetup(testClass, setOfPointsThatShouldNotBeSetup);
		...
		// ... and should *not* belong to the set of points that shouldn't be
		// set up
		setOfPointsThatShouldNotBeSetup.remove(staticCurrentPoint);
		staticAssertSetup(testClass, setOfPointsThatShouldBeSetup);
		staticAssertNotSetup(testClass, setOfPointsThatShouldNotBeSetup);
	}
	...
	static void staticAssertNotSetup(Class<? extends MultiLevelCleanupTest> testClass,
		EnumSet<CallbackPoint> setOfPointsThatShouldNotBeSetup) {
		getStaticResourceChecker(testClass).assertNotSetup(staticSoftly, staticCurrentPoint,
			setOfPointsThatShouldNotBeSetup);
	}
	...
	void verifyCurrentCallbackPoint(EnumSet<CallbackPoint> setOfPointsThatShouldBeSetup) {
		...
		// ... check that the remaining are *not* set up...
		assertNotSetup(setOfPointsThatShouldNotBeSetup);
		...
		// ... and should *not* belong to the set of points that shouldn't be
		// set up
		setOfPointsThatShouldNotBeSetup.remove(currentPoint);
		assertSetup(setOfPointsThatShouldBeSetup);
		assertNotSetup(setOfPointsThatShouldNotBeSetup);
	}
	...
	void assertNotSetup(EnumSet<CallbackPoint> setOfPointsThatShouldNotBeSetup) {
		getStaticResourceChecker(getClass()).assertNotSetup(softly, currentPoint, setOfPointsThatShouldNotBeSetup);
		getResourceChecker().assertNotSetup(softly, currentPoint, setOfPointsThatShouldNotBeSetup);
	}
	...
	// Note that the purpose of this test method is to verify the resource
	// setup/teardown
	@ParameterizedTest
	@ValueSource(ints = {
		1, 2, 3
	...
	void afterAfterEach() {
		...
		assertNotSetup(EnumSet.complementOf(shouldBeSetup));
	}
}
